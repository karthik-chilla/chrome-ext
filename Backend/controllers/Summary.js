...
async function summarise(req, res) {
  try {
    ...
    const prompt = `Summarize the following content in ${type === "short" ? "1-2 sentences" : "2-3 paragraphs"}:\n\n${truncatedText}`;
    ...
    if (totalSummaries > 100) {
      const leastUsedRecord = await Summary.findOne().sort({ lastAccessed: 1 });
      if (leastUsedRecord) {
